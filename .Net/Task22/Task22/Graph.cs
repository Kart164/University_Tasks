...
namespace Task22
{
    public class Graph
    {
        ...
        public void TaskFunc(int distance)
        {
            int[,] p;
            long[,] a = graph.Floyd(out p); //запускаем алгоритм Флойда
            int i, j;
            ...
            for (i = 0; i < graph.Size; i++)
            {
                for (j = 0; j < graph.Size; j++)
                {
                    if (i != j && !(vertexCheck.Contains(j.ToString() + i.ToString())))
                    {
                        if (a[i, j] != int.MaxValue && a[i, j]<=distance)
                        {
                            Queue<int> items = new Queue<int>();
                            items.Enqueue(i);
                            graph.WayFloyd(i, j, p, ref items);
                            ...
                            if (items.Count == 2)
                            {
                                Console.Write("можно построить дорогу между {0} и {1}, расстояние {2}", i, j, a[i, j]);
                                vertexCheck += i;
                                vertexCheck += j;
                                Console.WriteLine();
                            }
